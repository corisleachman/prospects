...
**25 Sep 2026: Stage 2c (import screen) shipped**
- New three-step import (Import list / ⌘K "Import a list"): **Upload & map** → **Review matches** → **Done**, using `import_suggest` / `import_commit` / `import_undo`. The classic import stays available from step 1 for "enrich existing".
- Mapping: agency, person and note field groups. Note fields (summary / type / source link / date) claim their columns first, so a `source_url` column isn't mistaken for the agency website. First/last name only map on an exact header. Identical notes for the same agency and date are kept once.
- Defaults: list for new people (required), tier for new agencies, source name shown on notes.
- Review: filter chips (needs a decision / all / new agencies / new people / existing / skipped), per-row choices, bulk quick decisions, "Import" blocked until nothing needs a decision. Recent imports listed with Undo.
- **End-to-end test** against a local PostgREST on a replay of live (real RPCs and RLS, 8 s statement timeout, ~1,200 seeded contacts), using the real 806-row Creative Boom file: suggest 2.7 s, commit 3.2 s, undo 2.5 s. Undo returned the database exactly to baseline. Correct handling of Koto / Heaps + Stacks / Born Social / Stereo (match), Ourselves, Koto APAC, ZEAL (possible) and Amy Searle (possible move). No JS errors.
